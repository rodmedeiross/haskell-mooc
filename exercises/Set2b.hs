...
countdown n = (count "Ready! " n) ++ "Liftoff!"
            where count s 0 = s
                  count s n = count (s ++ show n ++ "... ") (n - 1)
...
smallestDivisor n = smallestDivisor' 0 n n

smallestDivisor' :: Integer -> Integer -> Integer -> Integer
smallestDivisor' res 1 _ = res
smallestDivisor' res acc n
            | mod n acc == 0 = smallestDivisor' acc (acc-1) n
            | otherwise = smallestDivisor' res (acc -1) n
