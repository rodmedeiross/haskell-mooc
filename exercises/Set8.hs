...
showHexBin i = go i 3
  where
    go _ (-1) = []
    go i f
      | div i (16 ^ f) == 0 = '0' : go (mod i (16 ^ f)) (f - 1)
      | otherwise = (digit (div i (16 ^ f))) : go (mod i (16 ^ f)) (f - 1)
    digit x
      | x >= 0 && x < 16 = intToDigit x
      | otherwise = 'X'
...
blendColor (Color r g b) (Color rb gb bb) = Color (div (r + rb) 2) (div (g + gb) 2) (div (b + bb) 2)

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture fx) (Picture fy) = Picture (\c -> f (fx c) (fy c))
...
paintSolid color (Shape g) (Picture f) = Picture z
  where
    z coord
      | g coord = color
      | otherwise = f coord
...
paint (Picture f) (Shape s) (Picture g) = Picture z
  where
    z coord
      | s coord = f coord
      | otherwise = g coord
...
largeVerticalStripes = zoom 10 (flipXY (stripes red yellow))
...
instance Semigroup Color where
  (Color r g b) <> (Color r0 g0 b0) = Color (r + r0) (g + g0) (b + b0)

instance Transform Blur where
  apply _ (Picture f) = Picture z
    where
      z (Coord x y) = Color (div r 5) (div g 5) (div b 5)
        where
          (Color r g b) = (j <> k <> l <> t <> w)
          j = f (Coord x y)
          k = f (Coord (x - 1) y)
          l = f (Coord (x + 1) y)
          t = f (Coord x (y + 1))
