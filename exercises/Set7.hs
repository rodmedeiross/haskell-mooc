...
data State = Start | AE | AS | AF | ASF | AFS | AFM | Error | Finished
  deriving (Eq, Show)

step :: State -> Event -> State
step Start AddEggs = AE
step AE AddFlour = AF
step AE AddSugar = AS
step AS AddFlour = ASF
step AF AddSugar = AFS
step ASF Mix = AFM
step AFS Mix = AFM
step AFM Bake = Finished
...
average (n :| xs) = (n + sum xs) / fromIntegral (1 + length xs)
...
reverseNonEmpty (n :| xs) = foldl (\(acc :| accs) x -> x :| (acc : accs)) (n :| []) xs
...
passwordAllowed s (ContainsSome c) = any (`elem` c) s
passwordAllowed s (DoesNotContain d) = all (`notElem` d) s
...
data Arithmetic = Operand Integer | Summ Arithmetic Arithmetic | Mult Arithmetic Arithmetic
  deriving (Show)
...
operation o a b = case o of
  "+" -> Summ a b
  "*" -> Mult a b

evaluate :: Arithmetic -> Integer
evaluate (Summ a b) = (evaluate a) + (evaluate b)
evaluate (Mult a b) = (evaluate a) * (evaluate b)
evaluate (Operand a) = a

render :: Arithmetic -> String
render (Operand x) = show x
render (Summ a b) = "(" ++ (render a) ++ "+" ++ (render b) ++ ")"
