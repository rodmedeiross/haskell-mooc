...
treeMax t = go t 0
  where
    go Empty n = n
    go (Node x l r) n
      | x > n = max (go l x) (go r x)
      | otherwise = max (go l n) (go r n)
...
set ((StepL) : steps) v (Node x l r) = Node x (set steps v l) r
set ((StepR) : steps) v (Node x l r) = Node x l (set steps v r)
...
search s t = go s t []
  where
    go s Empty steps = Nothing
    go s (Node x l r) steps
      | s == x = Just $ reverse steps
      | go s l (StepL : steps) == Nothing = go s r (StepR : steps)
      | otherwise = go s l (StepL : steps)
