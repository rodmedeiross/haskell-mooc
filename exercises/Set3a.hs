...
capitalize = unwords . map caps . words
     where caps [] = []
...
multiCompose fs = go (reverse fs) 
            where 
                  go [] res = res
                  go (f:fs) res = go fs (f res)
------------------------------------------------------------------------------
...
interpreter commands = go commands 0 0
            where 
                go [] _ _  = []
                go (c:rest) x y = case c of 
                                                "up" -> go rest x (y+1)
                                                "down" -> go rest x (y-1)
                                                "left" -> go rest (x-1) y
                                                "right" -> go rest (x+1) y
                                                "printY"-> show y:go rest x y 
                                                "printX"-> show x:go rest x y 
                                                _ -> go rest x y
