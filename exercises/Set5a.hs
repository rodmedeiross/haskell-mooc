...
data BusTicket = MonthlyTicket String | SingleTicket deriving (Show)
...
buyOneMore (MkShoppingEntry n p q) = MkShoppingEntry n p (q + 1)
...
setName name (Person _ a) = Person name a
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix l r) = zipWith (\x y -> (x + y) / 2) (rgb l) (rgb r)
...
toBin = go
  where
    ...
    go x =
      let (q, r) = divMod x 2
       in (if r == 0 then O (go q) else I (go q))
