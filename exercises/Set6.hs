...
simplify (RationalNumber x y) = RationalNumber (div x f) (div y f)
  where
    f = (gcd x y)
...
class (Enum a) => Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 e = e
  stepMany i e = stepMany (i - 1) (step e)
...
instance Enum Color where
  ...
  succ a = toEnum $ mod ((fromEnum a) + 1) 3
  pred a = toEnum $ mod ((fromEnum a) - 1) 3
...
instance Enum Suit where
  ...
  succ a = toEnum $ mod ((fromEnum a) + 1) 4
  pred a = toEnum $ mod ((fromEnum a) - 1) 4
