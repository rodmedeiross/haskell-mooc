...
countValid = length . filter go
  where
    go (fst : scd : trd : fth : fif : sth : rest)
      | trd == fif = True
      | fth == sth = True
      | otherwise = False
...
repeated [] = Nothing
repeated (x : []) = Nothing
repeated (x : xs) = case x == head xs of
  True -> Just x
  False -> repeated xs
...
open code l@(Lock inner locked)
  | code == inner = Lock inner False
  | otherwise = l

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Lock c _) = (Lock c True)

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode c l@(Lock old locked)
  | locked = l
  | otherwise = Lock c locked
...
instance Eq Text where
  (==) (Text ss) (Text xs) = (filter (not . isSpace) ss) == (filter (not . isSpace) xs)
...
compose xs zs = mapMaybe (\(a, b) -> (a,) <$> lookup b zs) xs
